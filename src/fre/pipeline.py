...
def load(ticker: str, fiscal_years: list[int], snapshot_ids: dict[str, str] | None = None, as_of=None) -> Dataset:
    ...
    return normalize(cf, subs, snapshot_ids=ids, retrieved_at=snapshot.info(ids["companyfacts"])["retrieved_at"],
                     actions=actions_for(ticker), fiscal_years=fiscal_years, as_of=as_of)
