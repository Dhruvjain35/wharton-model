...
|---|---|
| `fre dossier GOOGL --peers MSFT META` | Fundamentals dossier (`dossier.md`, `tables/*.csv`, `run.json`) |
| `fre verify-run out/GOOGL/<run>/run.json` | Rebuilds the run from its pinned inputs and proves the output digest is identical |
| `fre value GOOGL` | Runs `reviews/GOOGL/valuation.yaml`: DCF, scenarios, reverse DCF, sensitivity grids |
| `fre screen GOOGL` | Writes `review.html`: charts plus click-through provenance for every figure |
| `fre verify-ledger GOOGL` | Checks every accounting-ledger quote against the filing text |
...
3. **Reconciliation gate.** Each reported fact is checked against the line of the filed financial statement it came from (SEC-rendered statement pages), at the statement's own precision, then against note tables. A mismatch blocks the fact.
...
7. **Zero is proven, not assumed.** A component the company does not have (for example no commercial paper) becomes 0 only when the engine confirms that no XBRL fact, statement line or note table shows it.
...
- Peer table with comparability notes established from the data. There is no composite score.
...
- Base, adverse and favorable scenarios; reverse DCF (bracketed root finding that reports "no solution" or "multiple solutions" rather than forcing one); growth × margin and WACC × terminal-growth grids; a capex break-even; a haircut on the holdings that carry the disputed gains
...
- [ ] Write the business-mix comparability note for MSFT and META from Item 1 of each 10-K
...
tests/              failure-mode tests (110+), including a hand-calculated DCF fixture
```
